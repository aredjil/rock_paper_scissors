...
void create_data_file(const std::string dir_path ="../data", const std:;string file_path="../data/results.csv"){

}
/**
  Callback function to dump the results
 */
PetscErrorCode Monitor(TS ts, PetscInt stepid, PetscReal t, Vec U, void *ctx)
{
    const PetscScalar *u;
    std::string dir = "../data"; // Path where to save the data assuming
                                 // the directory that contains the main function is in
                                 //./root/src/main.x
    std::string file_name = "../data/result.csv";
